...
class NewLifeUtils(object):
    ...
    def log(self, message, tag=''):
        ...
        print(self.formater(self.loggerPrintPattern_log, message, tag, self.loggerTagMaxLenght,
                            self.loggerDateTimeFormat_date, self.loggerDateTimeFormat_time,
                            additional=self.loggerColorMap))

    def wrn(self, message, tag=''):
        if tag == '':
            tag = self.loggerDefaultTag_wrn
        print(self.formater(self.loggerPrintPattern_wrn, message, tag, self.loggerTagMaxLenght,
                            self.loggerDateTimeFormat_date, self.loggerDateTimeFormat_time,
                            additional=self.loggerColorMap))
    ...
    def err(self, message, tag=''):
        ...
        print(self.formater(self.loggerPrintPattern_err, message, tag, self.loggerTagMaxLenght,
                            self.loggerDateTimeFormat_date, self.loggerDateTimeFormat_time,
                            additional=self.loggerColorMap))
    ...
    def tip(self, message, tag=''):
        ...
        print(self.formater(self.loggerPrintPattern_tip, message, tag, self.loggerTagMaxLenght,
                            self.loggerDateTimeFormat_date, self.loggerDateTimeFormat_time,
                            additional=self.loggerColorMap))
    ...
    def rea(self, message, tag=''):
        ...
        s += self.formater(self.loggerPrintPattern_rea, message, tag, self.loggerTagMaxLenght,
                           self.loggerDateTimeFormat_date, self.loggerDateTimeFormat_time, obj={'read': read},
                           additional=self.loggerColorMap)
        print(s)
        return read
    ...
    def setSeed(self, customseed='', display=''):
        def getRaw():
            rawseed = str(int((datetime.datetime.now().microsecond ** 3) * (datetime.datetime.now().second ** 10) + (
        ...
        def seedFormater(formated, displayTemplate=''):
            if displayTemplate != '':
                try:
                    ...
                    self.wrn(f'Reseting: "{displayTemplate}" -> "" ', self.SeedTag['wrn'])
                    displayTemplate = '4444'
            elif displayTemplate == '':
                displayTemplate = '4444'
                self.log(f'Using default display - {displayTemplate}', self.SeedTag['log'])
            else:
                self.log(f'Using non def display - {displayTemplate}', self.SeedTag['log'])
            need, i = 0, 0
            while i < len(displayTemplate):
                need += int(displayTemplate[i])
                i += 1
            self.log(f'Goted {len(formated)}', self.SeedTag['log'])
            self.log(f'Need seed with length > {need} (display {displayTemplate})', self.SeedTag['log'])
            ...
            while i < len(displayTemplate):
                ...
                formated_space += f'{formated[offset - int(displayTemplate[i]):offset]} '
                self.log(
        ...
        if customseed != '':
            ...
            self.log(f'{random.randrange(1, 100)};{random.randrange(1, 100)};{random.randrange(1, 100)};',
                     self.SeedTag['log'])
            self.log(f'{random.randrange(1, 100)};{random.randrange(1, 100)};{random.randrange(1, 100)};',
                     self.SeedTag['log'])
    ...
    def randselect(self, source, use_zero=False):
        ...
        return -1
    def my_cmd(command, param = None):
        if command.lower() == 'exit':
            print('123 this is exit')
...
def Demonstration():
    ...
    for b in range(1, 255, 6):
        for c in range(1, 255, 100):
            # noinspection PyTypeChecker
            s += nlu.FGC.BLACK + nlu.ACC.BCUSTOM(255, b,
                                                 c) + f'|255,{screate(b, 3)},{screate(c, 3)}' + nlu.ACC.RESET
            # noinspection PyTypeChecker
            s2 += nlu.FGC.BLACK + nlu.ACC.BCUSTOM(b, 255,
                                                  c) + f'|{screate(b, 3)},255,{screate(c, 3)}' + nlu.ACC.RESET
            # noinspection PyTypeChecker
            s3 += nlu.FGC.BLACK + nlu.ACC.BCUSTOM(b, c,
                                                  255) + f'|{screate(b, 3)},{screate(c, 3)},255' + nlu.ACC.RESET
    ...
    for b in range(1, 255, 6):
        for c in range(1, 255, 100):
            # noinspection PyTypeChecker
            s += nlu.ACC.BCUSTOM(0, 255 - b,
                                 255 - c) + f'|  0,{screate(255 - b, 3)},{screate(255 - c, 3)}' + nlu.ACC.RESET
            # noinspection PyTypeChecker
            s2 += nlu.ACC.BCUSTOM(255 - b, 0,
                                  255 - c) + f'|{screate(255 - b, 3)},  0,{screate(255 - c, 3)}' + nlu.ACC.RESET
            # noinspection PyTypeChecker
            s3 += nlu.ACC.BCUSTOM(255 - b, 255 - c,
                                  0) + f'|{screate(255 - b, 3)},{screate(255 - c, 3)},  0' + nlu.ACC.RESET
    ...
    for b in range(1, 255, 6):
        for c in range(1, 255, 100):
            # noinspection PyTypeChecker
            s += nlu.ACC.CUSTOM(0, 255 - b,
                                255 - c) + f'|  0,{screate(255 - b, 3)},{screate(255 - c, 3)}' + nlu.ACC.RESET
            # noinspection PyTypeChecker
            s2 += nlu.ACC.CUSTOM(255 - b, 0,
                                 255 - c) + f'|{screate(255 - b, 3)},  0,{screate(255 - c, 3)}' + nlu.ACC.RESET
            # noinspection PyTypeChecker
            s3 += nlu.ACC.CUSTOM(255 - b, 255 - c,
                                 0) + f'|{screate(255 - b, 3)},{screate(255 - c, 3)},  0' + nlu.ACC.RESET
...
def de(nlu,seed):
    ...
    for y in range(40):
        for x in range(160):
            ns = math.fabs(noise.pnoise3(float(x)*0.001, float(y)*0.001, float(seed), 6))
            #print(ns*255)
            ra=int(ns*255/0.1)
            s+=nlu.ACC.BCUSTOM(ra,ra,ra)+' '#┼
        print(s+nlu.ACC.RESET)
        s=''
...
def main():
    nlu = NewLifeUtils(silent = True)
    nlu.setSeed(nlu.rea('Enter seed: '),'6')
    sd = int(nlu.getSeed()['raw'])
    nlu.wrn(sd)
    import time
    time.sleep(1)
    de(nlu,sd)
