    ...
    def checkParamCount(self, command = ''):
        ...
        for rcommand in console.registeredCommands:  
            if rcommand["command"] == command:
                if self.paramCount > (len(rcommand['required'])+len(rcommand['optional'])) and self.paramCount < len(rcommand['required']):
                    return False
                break
        ...
        while self.exitState == "run":
            try:
                ...
                if self.command == 'fuck':
                    raise Exception('fatal','Why you so evil?...',':_(','TIP: you can be beter')
                self.runned = False
                ...
                if len(e.args) == 0:
                    ...
                    if 'warn' in e.args:
                        e.args = tuple(x for x in e.args if x not in ['fatal','notraceback','warn'])
                        err =\
                        f'\n----------------------------------\n'+\
                        f'Warning!\n\n'+\
                        f'More information:\n'+\
    ...
    class database(BasicModule.command):
        ...
        def run(console):
            if console.checkParamCount():
                ...
                dbmanager = BasicModule(console.nlu, 'DB Manage', 'easy way to manage database')
