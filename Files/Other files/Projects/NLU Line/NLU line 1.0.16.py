                ...
                def run(console):
                    if console.registeredFunctions["checkConnection"](console):
                        ...
                        try:
                            ...
                            execute = f"PRAGMA TABLE_INFO('{table}')"
                            console.cursor.execute(execute)
                            rows = len(list(console.cursor.fetchall())) 
                            console.logger.wrn(f"response: {response}")
                            console.logger.wrn(f"ROWS: {rows}")
                            lineLengths = []
                            for i in range(rows):
                                lineLengths.append(0)
                            for line in response:
                                for row in range(rows):
                                    if lineLengths[row] < len(line):
                                        console.logger.wrn(f"{line}")
                                        
                                        lineLengths[row] = len(line)
                            console.logger.wrn(f"{lineLengths}")
