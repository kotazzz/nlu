                ...
                if self.command == "":
                    ...
                    for command in self.registeredCommands:
                        if self.command in command["aliases"]:
                            ...
                            if not(self.paramCount > (len(self.running['required'])+len(self.running['optional'])) or self.paramCount < len(self.running['required'])) or self.running['dontcheckopt']:
                                command["run"](self)
                            else:
                                self.invalidUsage()
                            self.runned = True
                            ...
                            if (len(console.parametrs)%2==1):
                                ...
                                for fieldtype in fieldtypes:
                                    if fieldtype not in ['text','int']:
                                        console.logger.wrn(f"Wrong field type: {fieldtype}")
                                        break
                                else:
                                    console.logger.tip(f"Create table '{name}'")
                                    fields = []
                                    for fieldnum in range(len(fieldnames)):
                                        fields.append(f'{fieldnames[fieldnum]} {fieldtypes[fieldnum]}')
                                    execute = f'CREATE TABLE {name} ({", ".join(fields)})'
                                    console.logger.tip(f"Executing: {execute}")
                                    try:
                                        console.cursor.execute(execute)
                                    except console.sqlite3.OperationalError:
                                        console.logger.wrn(f"'{name}' is already exists!")
                                    else:
                                        console.logger.log(f"Succeful created table {name}!")
                            else:
                                console.logger.wrn(f"Something wrong: not given the name of a table or field is incorrectly documented")
                        else:
                            console.logger.wrn(f"not given the name of a table or field is incorrectly documented")
                    else:
            ...
            class adddata(BasicModule.command):
                ...
                def run(console): 
                    ...
                    if len(fields) > len(response):
                        console.logger.wrn(f'Too many fields to add; Need: {len(response)}, Provided: {len(fields)}')
                    if len(fields) < len(response):
                         console.logger.wrn(f'too few fields to add; Need: {len(response)}, Provided: {len(fields)}')
                    else:
