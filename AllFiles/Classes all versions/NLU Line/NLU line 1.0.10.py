        ...
        def run(console):
            ...
            class describe(BasicModule.command):
                ...
                def run(console): 
                    ...
                    console.logger.log(f'Result')
                    response = console.cursor.fetchall()
                    result = ''
                    sizes = [3,20,15,7,20,2]
                    result+='{screate("cid",sizes[rowid])}{screate("name",sizes[rowid])}{screate("type",sizes[rowid])}{screate("notnull",sizes[rowid])}{screate("default",sizes[rowid])}{screate("pk",sizes[rowid])}'
