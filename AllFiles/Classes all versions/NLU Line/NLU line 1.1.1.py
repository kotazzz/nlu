...
class command:
        ...
        def run(console):
            ...
            console.logger.log("command = 'mycommand'")
            console.logger.log("description = 'This is my custom command'")
            console.logger.log("aliases = [command, 'mycommandalias']")
            console.logger.log("required = ['myreq']")
            console.logger.log("optional = ['myopt']")
            console.logger.log("disablecheck = False")
class Action:
    name = 'MyAction'
    def execute():
        console.logger.log("The action executed!")
...
class BasicModule(object):
    def __init__(self, nlu, cmdname="root", cmdabout="default console"):
        ...
        # Setting up run state as "run"
        self.runState = "run"

    def registerAction(self, actionClass):
        self.registeredActions[name] = actionClass

    def registerExitAction(self, exitActionClass):
        self.registeredExitActions.append(functuion)

    def registerInitAction(self, initActionClass):
        ...
        self.registeredCommands.append(
            {
                ...
                "disablecheck": commandClass.disablecheck,
    ...
    def __initBasicCommand(self):
        ...
        class _hello(self.command):
            command = "hello"
            description = "builtin command, that can show, how BasicModule api works"
            aliases = [command, "hi"]
            required = []
            optional = ["name"]
    ...
    def __parseInput(self, readed):
        ...
        tempResult = []
        for item in result:
            tempResult.append(re.sub(r"\B'|\b'", "", item))
        result = [x for x in tempResult if x != ""]
        if len(result) == 0:
    ...
    def ExceptionPrint(self, e, additional = ['notb','err']):
        if additional not in ['notb','err','wrn']:
            additional =  ['err']
    ...
    class database(BasicModule.command):
        ...
        def run(console):
            ...
            def createTable(console, rowCount, sizes, data, title="TABLE"):
                ...
                for datatext in range(0, len(data), rowCount):  # 4444444
                    for i in range(rowCount):
                        table += f"{screate(data[datatext+i], sizes[i])}║"
                    table += "\n║"
                table = table[:-1] + "╚"
            ...
            def getMaxSizeRecord(console, response, rowsCount, header):

                lineLengths = []  # Вычисляем начальную длину названия поля
                for i in range(rowsCount):
                    lineLengths.append(len(header[i]))
                ...
                def run(console):
                    if console.registeredFunctions["checkConnection"](console):
                        if console.paramCount == 1:
                            ...
                            console.logger.log(f"Executing: {execute}")
                            console.cursor.execute(execute)
                            tableData = console.cursor.fetchall()
                            if tableData == []:
                                console.logger.wrn(f"No table named '{table}'")
                            else:
                                rowsCount = 6
                                headers = [
                                    "Unical id",
                                ...
                                console.logger.tip(len(tableData[0]))
                                maxLengths = console.registeredFunctions[
                                    "getMaxSizeRecord"
                                ](console, tableData, rowsCount, headers)

                                data = headers
                                for line in tableData:
                                    for el in line:
                                        data.append(el)
                                console.registeredFunctions["createTable"](
                                    console, rowsCount, maxLengths, data
                                )
                ...
                def run(console):
                    if console.registeredFunctions["checkConnection"](console):
                        try:
                            ...
                            tableInfo = list(console.cursor.fetchall())
                            rowsCount = len(tableInfo)  # Количество столбцов

                            # Вычисляем максимальную длину столбца
                            headerStrings = []
                            for i in range(rowsCount):
                                headerStrings.append((str(tableInfo[i][1])))
                            lineLengths = console.registeredFunctions[
                                "getMaxSizeRecord"
                            ](console, tableData, rowsCount, headerStrings)

                            data = headerStrings  # Начинаем собирать шапку

                            for record in tableData:  # Для записи в таблице
                            ...
                            console.registeredFunctions["createTable"](
                                console, rowsCount, lineLengths, data
                            )
            ...
            class tablelist(BasicModule.command):
                ...
                command = "tablelist"
                description = "displays tables in base"
                aliases = [command]
                required = []
                optional = []
                ...
                def run(console):
                    if console.registeredFunctions["checkConnection"](console):
                        ...
                        console.logger.log(f"Executing: {execute}")
                        console.cursor.execute(execute)
                        tableList = list(console.cursor.fetchall())
                        console.logger.log(f"resp: {tableList}")
            ...
            dbmanager.registerCommand(tablelist)
