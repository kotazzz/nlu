...
class NewLifeUtils(object):
    def __init__(self, silent=False):
        ...
        self.currentDirectory = self.libsManager_os.getcwd()
    ...
    def loggerFormatter(
        ...
        return f"{self.Color_ACC.RESET}{self.Color_MCC.REWRITELINE}" + pattern.format(
            **self.loggerColorMap,
            **additional,
    ...
    def tracebackManagerExceptionPrint(self, exception, exceptionType="err", tb=True):
        ...
        if exceptionType == "attention":
            self.loggerLog(
                errorText.replace(
                    "{ExceptionTitle}", self.stringManagerScreate("Attention!", 20)
                )
            )
        if exceptionType == "wrn":
            self.loggerWrn(
                errorText.replace(
                    "{ExceptionTitle}", self.stringManagerScreate("Warning!", 20)
                )
            )
        elif exceptionType == "err":
            self.loggerErr(
                errorText.replace(
                    "{ExceptionTitle}", self.stringManagerScreate("Error!", 20)
                )
            )
        elif exceptionType == "fatal":
            self.loggerErr(
                errorText.replace(
                    "{ExceptionTitle}", self.stringManagerScreate("Fatal Error!", 20)
                )
            )
            exit(-1)
        else:
            self.loggerErr(
                errorText.replace(
                    "{ExceptionTitle}",
                    self.stringManagerScreate("Something wrong...", 20),
                )
            )

    ########################## TRACEBACKMANAGER #######################
    ########################## TABLEMANAGER   #######################
    def tableManagerCreateTable(
        self,
        rowCount,
        sizes,
        data,
    ...
    def tableManagerCreateBigTable(
        self,
        rowCount,
        sizes,
        data,
            ...
            for row in range(0, rowCount):
                for blockLineNum in range(len(data[line + row])):
                    ...
                    try:
                        block[
                            blockLineNum
                        ] += f"{tableElement[3]}{self.stringManagerScreate(data[line+row][blockLineNum],sizes[row], align)}"
                    except:
                        block.append(
                            f"{tableElement[3]}{self.stringManagerScreate(data[line+row][blockLineNum],sizes[row], align)}"
                        )
    ...
    class consoleShellManagerModule(object):
        ...
        def main(self):
            ...
            while self.runState == "run":
                try:
                    _ = self.nlu.stringManagerParseArgs(
                        self.nlu.loggerRea(f"{self.cmdname.title()} >")
                    )
...
########################## FILEMANAGER         ##################
    ...
    def debug(self):
        self.loggerLog(
            f"{self.Color_FGC.PURPLE}------------------ debug info  -----------------{self.Color_ACC.RESET}"
        )
        ...
        self.loggerLog(
            f"{self.Color_FGC.BBLUE} Color Out:         {self.Color_FGC.CYAN}HI!{self.Color_ACC.RESET}"
        )
        self.loggerLog(
            f"{self.Color_FGC.PURPLE}------------------ debug info  -----------------{self.Color_ACC.RESET}"
        )
