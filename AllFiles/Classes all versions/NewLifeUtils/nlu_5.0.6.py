...
class NewLifeUtils(object):
    def __init__(self, silent = False):
        ...
        if not silent:
            ...
            suspended = f'{self.Color_FGC.BBLUE }Suspended'
...
######################### LOGGER ##################################
    ...
    def loggerRea(self, message, tag=""):
        if message[-1] not in [' ', '>', ':']:
            message += ': '
        print(f"{self.Color_BCUSTOMRGB(0, 43, 112)}{self.Color_CUSTOMRGB(235, 54, 30)}{message}", end = '')
        read = input()
...
########################## TRACEBACKMANAGER #######################
    def tracebackManagerExceptionPrint(self, exception, exceptionType = 'err', tb = True):
        ...
        if exceptionType == 'attention':
            self.loggerLog(errorText.replace('{ExceptionTitle}', self.stringManagerScreate('Attention!', 20)))
        if exceptionType == 'wrn':
            self.loggerWrn(errorText.replace('{ExceptionTitle}', self.stringManagerScreate('Warning!', 20)))
        elif exceptionType == 'err':
            self.loggerErr(errorText.replace('{ExceptionTitle}', self.stringManagerScreate('Error!', 20)))
        elif exceptionType == 'fatal':
            self.loggerErr(errorText.replace('{ExceptionTitle}', self.stringManagerScreate('Fatal Error!', 20)))
            exit(-1)
        else:
            self.loggerErr(errorText.replace('{ExceptionTitle}', self.stringManagerScreate('Something wrong...', 20)))
...
########################## CONSOLESHELLMANAGER ##################
    ...
    class consoleShellManagerModule(object):
        ...
        class consoleShellManagerTask:
            def execute(console):
                console.nlu.loggerLog(f'This is a event task')
            
        class consoleShellManagerFunction:
            name = 'mygf'
            def execute(console):
                console.nlu.loggerLog(f'This is a global function')
        ...
        def __init__(self,nlu, name = 'root', about = 'default console', settingsfile = ''):
            ...
            self.nlu.loggerTagMaxLenght = len(self.cmdname)+5
            self.nlu.loggerLogDefaultTag = f'[L] {self.cmdname.title()}'
            self.nlu.loggerWrnDefaultTag = f'[W] {self.cmdname.title()}'
            self.nlu.loggerErrDefaultTag = f'[E] {self.cmdname.title()}'
            self.nlu.loggerTipDefaultTag = f'[T] {self.cmdname.title()}'
            self.nlu.loggerReaDefaultTag = f'[R] {self.cmdname.title()}'
            self.nlu.loggerReaFormat = "{white}[{time}] {blue}{tag}{empty}{reset}: {cyan}{read}{reset}"
            
            self.runState = 'setup'

        class __cls(consoleShellManagerCommand):
            command = "cls"
            ...
            def run(console):
                console.nlu.libsManager_os.system("cls")

        class __help(consoleShellManagerCommand):
            command = "help"
            ...
            def run(console):
                class CLR:
                    MDL = console.nlu.Color_FGC.RED
                    MDLDSK = console.nlu.Color_FGC.BRED
                    CMD = console.nlu.Color_FGC.BLUE
                    CMDDSK = console.nlu.Color_FGC.CYAN
                    ALS = console.nlu.Color_FGC.PURPLE
                    ALSTXT = console.nlu.Color_FGC.BPURPLE
                    STTL = console.nlu.Color_FGC.GREEN
                    SCMD = console.nlu.Color_ACC.UNDERLINE + nlu.Color_FGC.WHITE
                    SREQ = console.nlu.Color_FGC.BGRAY
                    SOPT = console.nlu.Color_FGC.GRAY
                    R = console.nlu.Color_ACC.RESET
                ...
                if console.paramCount == 0:
                    ...
                    console.nlu.loggerTip(helpPage, f"{console.cmdname} HELP")
        ...
        class __hello(consoleShellManagerCommand):
            command = "hello"
            ...
            def run(console):
                if console.paramCount == 1:
                    console.nlu.loggerLog(f"Hello, {console.parametrs[0]}")
                else:
                    console.nlu.loggerLog(f"Hello, world!")
        ...
        class __exitDefaultTask(consoleShellManagerTask):
            def execute(console):
                console.nlu.loggerLog(f'Exit with code: {console.runState}, run command: {console.run}')
        class __initDefaultTask(consoleShellManagerTask):
            def execute(console):
                console.nlu.loggerLog(f'Welcome to {console.cmdname}')
        ...
        def invalidUsage(self, command):
            ...
            class CLR:
                SCMD = self.nlu.Color_ACC.UNDERLINE + nlu.Color_FGC.WHITE
                SREQ = self.nlu.Color_FGC.BGRAY
                SOPT = self.nlu.Color_FGC.GRAY
                R = self.nlu.Color_ACC.RESET
            ...
            if command["optional"] != []:
                syntax += f'{CLR.SOPT}[{"] [".join(command["optional"])}]{CLR.R} '
            self.nlu.loggerWrn(f"Invalid usage. Syntax: {syntax}")
        ...
        def main(self):
            ...
            while self.runState == 'run':
                try:
                    _ = self.nlu.stringManagerParseArgs(self.nlu.loggerRea(f'{self.cmdname.title()} >'))
                    self.command = _['command']
                    self.parametrs = _['param']
                    self.paramCount = len(self.parametrs)
                    
                    if self.command == "fuck":
                        nlu.tracebackManagerExceptionPrint( Exception(
                            "Why you so evil?...",
                            ":_(",
                            "TIP: you can be beter",
                        ),  exceptionType = 'wrn', tb = False )
                    elif self.command != '':
                        for registered in self.registeredCommands:
                            if registered['command']== self.command:
                                self.run = registered
                                if (not (self.paramCount> (len(self.run["required"])    + len(self.run["optional"])) or self.paramCount < len(self.run["required"])
                            ...
                            self.nlu.loggerWrn('Unknown Command, type "help"')
                   
                except Exception as e:
                    self.nlu.tracebackManagerExceptionPrint(e)
