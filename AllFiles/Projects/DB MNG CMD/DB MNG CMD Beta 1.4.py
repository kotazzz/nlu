        ...
        for commandObj in commandsList:
            if commandObj == command:
                ...
                if commandsObject[command]["usage"]["required"] != []:
                    syntax += (
                        f' <{"> <".join(commandsObject[command]["usage"]["required"])}>'
                    )
                if commandsObject[command]["usage"]["optional"] != []:
                    syntax += (
                        f' [{"] [".join(commandsObject[command]["usage"]["optional"])}]'
                    )
                return syntax
        ...
        if helpType == "A":
            ...
            for moduleName in moduleKeys:
                ...
                for commandName in commandList:
                    ...
                    if (
                        storage[moduleName]["COMMANDS"][commandName]["usage"][
                            "required"
                        ]
                        != []
                    ):
        ...
        class help:
            ...
            def run(self):
                if self.paramCount == 0:
                    self.logger.tip(
                        self.createHelpPage(
                            self.nlu, self.storage["HELPPAGEDATA"], self.path, "M"
                        )
                    )
                elif self.paramCount == 1:
                    ...
                    for moduleName in moduleNames:
                        commandNames += list(
                            self.storage["HELPPAGEDATA"][moduleName]["COMMANDS"].keys()
                        )
                    if self.parametrs[0] in moduleNames:
                        self.logger.tip(
                            self.createHelpPage(
                                self.nlu,
                                self.storage["HELPPAGEDATA"],
                                self.parametrs[0],
                                "M",
                            )
                        )
                        ...
                        self.logger.tip(
                            self.createHelpPage(
                                self.nlu,
                                self.storage["HELPPAGEDATA"][self.path],
                                self.parametrs[0],
                                "C",
                            )
                        )
    ...
    def Main(self):
        ...
        while self.exitState == "run":
            ...
            for registeredCommand in self.commands:
                ...
                if self.command in self.getAliases(
                    self.storage["HELPPAGEDATA"][self.path],
