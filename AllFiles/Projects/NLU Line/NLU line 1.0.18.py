                ...
                def run(console):
                    if console.registeredFunctions["checkConnection"](console):
                        try:
                            ...
                            rowsNamesData = list(console.cursor.fetchall()) #PRAGMA TABLE_INFO('{table}') #описание таблицы
                            rowsCount = len(rowsNamesData) #Количество столбцов
                            ...
                            rowsNames = [] #Получаем названия полей                        
                            for i in range(rowsCount):
                            ...
                            for line in response: #В каждой записи
                                for row in range(rowsCount): #Для каждого столбца
                                    if len(str(line[row])) > lineLengths[row]: #Если длина строки в записи[столбце] больше
                                       lineLengths[row] = len(str(line[row])) #Устанавливаем ее как максимальную для столбца         
                            ...
                            data = rowsNames #Начинаем собирать шапку
                            for record in response: #Для записи в таблице
                                for element in record: #Для каждого элемента
                                    data.append(element) #добовляем его в общую массу, генератор сам разберется

                            createTable(rowsCount,lineLengths,data)
                        except Exception as e:
