                    ...
                    if console.registeredFunctions["checkConnection"](console):
                        ...
                        try:
                            ...
                            if e.args[0][0:13] == 'no such table':
                                ...
                                console.logger.err(e.args)
                        else:
                            response = console.cursor.fetchall() # SELECT * FROM {table}
                            execute = f"PRAGMA TABLE_INFO('{table}')"
                            console.cursor.execute(execute)
                            rowsNamesData = list(console.cursor.fetchall()) #PRAGMA TABLE_INFO('{table}')
                            rowsCount = len(rowsNamesData)
                            ...
                            for i in range(rowsCount):
                                rowsNames.append(rowsNamesData[i][1])
                                
                            for line in response:
                                for row in range(rowsCount):
                                    #console.logger.wrn (f"{line}: row({row}): {line[row]} len: {len(str(line[row]))}")
                                    if lineLengths[row] < len(str(line[row])):
                                        console.logger.wrn(f"{line}")
                                        lineLengths[row] = len(str(line[row]))
                            console.logger.wrn(f"new lineLengths: {lineLengths}")            
                            
                            def createTable(rowCount, sizes, data, title = 'TABLE'):
                                def screate(s, n):
                                    space = " " * (int(n) - len(str(s)))
                                    return space + str(s)
                                # ╔═╦═══╗
                                ...
                                for text in range(rowCount):#222222
                                    ttext +=screate(data[text], sizes[text])+'║'
                                ttext += '\n╠'
                                ...
                                for datatext in range(0,len(data),rowCount):#4444444 
                                    for i in range(rowCount):
                                        ttext += f'{screate(data[datatext+i], sizes[i])}║'
                                    ttext  += '\n║'
                                ttext = ttext[:-1]+'╚'
                                for i in range(rowCount):#1111111
                                    ttext += sizes[i]*'═'+'╩'
                                ...
                                console.logger.wrn(f"{title}:\n{ttext}")   #4444444      
                            console.logger.wrn(f"rowsCount: {rowsCount};\nresponse: \n{response}")
                            ...
                            for record in response:
                                for element in record:
                                    data.append(element)
                            print(element)
                            createTable(rowsCount,lineLengths,['idrow','textrow','1','hello','23','hi4'])
